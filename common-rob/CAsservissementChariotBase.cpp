...
CAsservissementChariotBase::CAsservissementChariotBase(){}
CAsservissementChariotBase::~CAsservissementChariotBase(){}

void CAsservissementChariotBase::Init(void)
{
	//Input
...
void CAsservissementChariotBase::Asser_chariot(void)
{
	//position
    codeur_position_chariot=getPositionCodeur();
	...
	if(	etat_asser_chariot==STOP)
	{
        ...
        commandeMoteur(commande_moteur_chariot);
		}
	}
		...
		switch(etat_recalage_butee)
		{
            ...
            case EN_COURS: // Apprentissage demandee, on demande un déplacement doucement jusqu'à  vitesse nulle avec tempo de confirmation
				...
				else
				{
                    ...
                    resetPositionCodeur(butee_basse);

					setConsigne(0);
				}

                if((isButeeBasse() /*TODO: A VERIFIER*/)&&(vitesse_consigne<0))
				{
					if(commande_moteur_chariot!=0.0)
					{
						commande_moteur_chariot=0.0;
                ...
                if((isButeeHaute() /*TODO: A VERIFIER*/)&&(vitesse_consigne>0))
				{
					if(commande_moteur_chariot!=0.0)
					{
						commande_moteur_chariot=0.0;
...
void CAsservissementChariotBase::Regul_chariot(void) // Régulateutr de vitesse PI
{
...
void CAsservissementChariotBase::setConsigne(int pos)
{
	int pos_seuil=pos;
...
void CAsservissementChariotBase::Stop_Chariot(void)
{
	etat_asser_chariot=STOP;
}

void CAsservissementChariotBase::Recal_Chariot(void)
{
	etat_asser_chariot=DEPLACEMENT;
...
void CAsservissementChariotBase::setCommandeMax(float calib){commande_chariot_max_C=calib;}
void CAsservissementChariotBase::setGainPosVit(float calib){gain_position_vitesse_C=calib;}
void CAsservissementChariotBase::setGainP(float calib){gain_prop_C=calib;}
void CAsservissementChariotBase::setGainI(float calib){gain_int_C=calib;}
void CAsservissementChariotBase::setSeuilConv(float calib){seuil_conv_C=calib;} //seuil cm ramene en pas codeur

bool CAsservissementChariotBase::isConverged()
{
    return (etat_asser_chariot == CONVERGE);
}
