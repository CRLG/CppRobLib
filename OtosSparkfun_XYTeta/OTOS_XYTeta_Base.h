...
class OtosXYTetaBase
{
  public:
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getVersionInfo(otos_version_t &hwVersion, otos_version_t &fwVersion);

    /// @brief Performs a self test of the OTOS
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t selfTest();
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t calibrateImu(uint8_t numSamples = 255, bool waitUntilDone = true);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getImuCalibrationProgress(uint8_t &numSamples);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getLinearScalar(float &scalar);
    ...
    /// @param config Signal processing configuration
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t setSignalProcessConfig(otos_signal_process_config_t &config);
    ...
    /// @param pose Offset of the sensor relative to the center of the robot
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t setOffset(otos_pose2d_t &pose);

    /// @brief Gets the position measured by the OTOS
    /// @param pose Position measured by the OTOS
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getPosition(otos_pose2d_t &pose);
    ...
    otos_error_t getPositionStdDev(otos_pose2d_t &pose);
    ...
    /// the covariance matrices of the Kalman filters used in the firmware, so
    /// they are just statistical quantities and do not represent actual error!
    otos_error_t getVelocityStdDev(otos_pose2d_t &pose);
    ...
    /// the covariance matrices of the Kalman filters used in the firmware, so
    /// they are just statistical quantities and do not represent actual error!
    otos_error_t getAccelerationStdDev(otos_pose2d_t &pose);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getPosVelAcc(otos_pose2d_t &pos, otos_pose2d_t &vel, otos_pose2d_t &acc);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getPosVelAccStdDev(otos_pose2d_t &pos, otos_pose2d_t &vel, otos_pose2d_t &acc);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    otos_error_t getPosVelAccAndStdDev(otos_pose2d_t &pos, otos_pose2d_t &vel, otos_pose2d_t &acc,
  ...
  protected:
    ...
    otos_error_t readPoseRegs(uint8_t reg, otos_pose2d_t &pose, float rawToXY, float rawToH);

    // Function to write raw pose registers and convert from specified units
    otos_error_t writePoseRegs(uint8_t reg, otos_pose2d_t &pose, float xyToRaw, float hToRaw);
    ...
    virtual otos_error_t writeRegister(uint8_t reg, uint8_t *buff, uint16_t size) = 0;
    virtual otos_error_t readRegister(uint8_t reg, uint8_t *buff, uint16_t size, uint16_t &byte_read) = 0;
    otos_error_t writeRegister(uint8_t reg, uint8_t value);
    otos_error_t readRegister(uint8_t reg, uint8_t &value);
